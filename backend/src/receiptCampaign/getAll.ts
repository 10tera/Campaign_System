...
export default router.get("/getAll", async (req, res) => {
    logger.info(`Access to /receiptCampaign/getAll`);
    try {
        let data:any = [];
        fs.readdir(`config/receiptCampaign`,(err,files) => {
            if(err){
                logger.error(err);
            ...
            files.forEach(file => {
                if(file.endsWith(".json") || file.endsWith(".JSON")){
                    const fileData = fs.readFileSync(`config/receiptCampaign/${file}`,"utf-8");
