...
export default router.post("/create", async (req, res) => {
    ...
    try {
        ...
        fs.writeFile(`config/receiptCampaign/${req.body.data.companyId.toString()}.json`, dataStr, { encoding: "utf-8" },async(err) => {
            ...
            const [] = await connection.query(`CREATE TABLE receiptCampaign_${req.body.data.companyId.toString()}(id int AUTO_INCREMENT PRIMARY KEY, uid varchar(255) not null, imgPath varchar(255) not null, state tinyint not null, prize varchar(255) not null, comment varchar(255) not null, date varchar(255) not null)`);
