...
export default router.post("/reApply", upload.single("img"), async (req, res) => {
    logger.info(`Access to /receiptCampaign/reApply`);
    let connection: any;
    try {
        ...
        const [row,fields] = await connection.execute(`SELECT * from receiptcampaign_${req.body.campaignId} where uid = ? and state = 1 limit 1`, [req.body.uid]);
        if (row.length === 0) {
            res.status(401).send("既に応募済みです。");
            return;
        }
        const [] = await connection.execute(`UPDATE receiptcampaign_${req.body.campaignId} set state = 0 where uid = '${req.body.uid}'`);
