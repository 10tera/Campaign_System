...
export default router.post("/reApply", upload.single("img"), async (req, res) => {
    ...
    try {
        ...
        const [row,fields] = await connection.query(`SELECT * from receiptcampaign_${req.body.campaignId} where uid = ? and state = 1 limit 1`, [req.body.uid]);
        if (row.length === 0) {
            res.status(401).send("既に応募済みです。");
            return;
        }
        const [] = await connection.query(`UPDATE receiptcampaign_${req.body.campaignId} set state = 0 where uid = '${req.body.uid}'`);
