...
export default router.put("/changeActive", async (req, res) => {
    logger.info(`Access to /instantWin/changeActive`);
    try {
        if (!(req.body.companyId)) {
            res.status(401).send("パラメータが不足しています。");
            return;
        }
        const roadFile = fs.readFileSync(`config/instantWin/${req.body.companyId.toString()}.json`, "utf-8");
        ...
        fs.writeFile(`config/instantWin/${req.body.companyId.toString()}.json`, afterData, { encoding: "utf-8" }, (err) => {
            if (err) {
