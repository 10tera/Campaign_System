...
export default router.post("/create", async (req, res) => {
    ...
    try {
        ...
        fs.writeFile(`config/instantWin/${req.body.data.companyId.toString()}.json`, dataStr, { encoding: "utf-8" }, async (err) => {
            ...
            const [] = await connection.execute(`CREATE TABLE instantwin_${req.body.data.companyId.toString()}(id int AUTO_INCREMENT PRIMARY KEY, uid varchar(255) not null, imgPath varchar(255) not null, state tinyint not null, comment varchar(255) not null, date varchar(255) not null)`);
