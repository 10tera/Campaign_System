...
export default router.post("/create", async (req, res) => {
    ...
    try {
        ...
        fs.writeFile(`config/questionnaire/${req.body.companyId.toString()}.json`, dataStr, { encoding: "utf-8" }, async (err) => {
            ...
            const [] = await connection.query(`CREATE TABLE questionnaire_${req.body.companyId.toString()}(id int AUTO_INCREMENT PRIMARY KEY, uid varchar(255) not null, ${dbInitStr} date varchar(255) not null)`);
