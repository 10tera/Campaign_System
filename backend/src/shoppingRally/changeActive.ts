...
export default router.put("/changeActive", async (req, res) => {
    logger.info(`Access to /shoppingRally/changeActive`);
    try {
        if (!("companyId" in req.body && "isActive" in req.body)) {
            res.status(401).send("パラメータが不足しています。");
            return;
        }
        const roadFile = fs.readFileSync(`config/shoppingRally/${req.body.companyId.toString()}.json`, "utf-8");
        ...
        fs.writeFile(`config/shoppingRally/${req.body.companyId.toString()}.json`, afterData, { encoding: "utf-8" }, (err) => {
            if (err) {
