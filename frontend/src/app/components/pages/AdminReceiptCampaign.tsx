...
import { receiptCampaignGetAll ,receiptCampaignChangeActive,receiptCampaignDelete,receiptCampaignGet, receiptCampaignGetUserInfo_NoQuery,ReceiptCampaignChangeUserState} from "../../api/ApiClient";
...
export const AdminReceiptCampaign = () => {
    ...
    const [searchingCampaign,setSearchingCampaign] = useState<ReceiptCampaignType>();
    ...
    const handleSearchButtonClick = async() => {
        ...
        if(!isOk)return;
        try {
            const allData = await receiptCampaignGet({ id: campaignId });
    ...
    const handleDownloadCsv = () => {
        ...
        for (let i = 0; i < searchedAllData?.length; i++) {
            let prizeTitle = "can not find";
            for (let j = 0; j < searchingCampaign.prizes.length; j++) {
                if (searchingCampaign.prizes[j].id.toString() === searchedAllData[i].prize) {
                    prizeTitle = searchingCampaign.prizes[j].title;
                }
            }
            let stateStr = "error";
            ...
            csvData += `${searchedAllData[i].id},${searchedAllData[i].uid},${prizeTitle},${searchedAllData[i].date},${stateStr},${searchedAllData[i].comment}\n`;
        }
    ...
    const handleChangeActive = async() => {
        ...
        try {
            const res = await receiptCampaignChangeActive({companyId:searchingCampaign?.companyId,isActive:!searchingCampaign?.isActive});
            window.alert("変更しました。");
    ...
    const handleDeleteCampaign = async() => {
        ...
        try {
            const res = await receiptCampaignDelete({companyId:searchingCampaign.companyId});
            window.alert("削除しました。");
            navigate("/admin/mypage");
        } catch (e) {
            console.error(e);
    ...
    const handleOpenModal = async(uid:string) => {
        ...
        try {
            const data = await receiptCampaignGetUserInfo_NoQuery({ id: searchingCampaign.companyId, uid: uid });
            setIsOpen(true);
            setUserInfo(data);
    ...
    const handleChangeState_to1 = async() => {
        ...
        try {
            const res = await ReceiptCampaignChangeUserState({ id: userInfo[0].id, comment: comment, state: 1, companyId: searchingCampaign?.companyId });
            window.alert("更新しました。");
    ...
    const handleChangeState_to2 = async() => {
        ...
        if (searchingCampaign?.companyId === undefined) return;
        try {
            const res = await ReceiptCampaignChangeUserState({ id: userInfo[0].id, comment: comment, state: 2, companyId: searchingCampaign?.companyId });
            window.alert("更新しました。");
            handleSearchButtonClick();
            handleModalClose();
        } catch (e) {
            ...
            <div css={mainDivCss}>
                ...
                <TableContainer>
                    <TableBody>
                        ...
                        <TableRow>
                            <TableCell align={"right"}>レシートキャンペーン</TableCell>
                            <TableCell>
                                <FormControl>
                                    {/*
                                        eslint-disable-next-line
                                    */}
                                    <Select value={campaignId} onChange={handleCampaignSelectChange} label={"レシートキャンペーン"}>
                                        {
                                            (() => {
                                                let ele: any = [];
                                                for (let i = 0; i < data.length; i++) {
                        ...
                        <TableRow>
                            ...
                            <TableCell align={"right"}>
                                <FormGroup>
                                    ...
                                    <FormControlLabel control={<Checkbox checked={checked_state2} onChange={handleChecked_state2_Change}/>} label={"承諾済み"} />
                                </FormGroup>
                            </TableCell>
                        </TableRow>
                    </TableBody>
                            ...
                            <TableContainer>
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell align={"right"}>賞品</TableCell>
                                                        ...
                                                        <TableRow key={`userdata-${i}`}>
                                                            ...
                                                            <TableCell align={"right"}>{
                                                                (() => {
                                                                    for (let j = 0; j < searchingCampaign.prizes.length; j++) {
                                                                        if (searchingCampaign.prizes[j].id.toString() === searchedAllData[i].prize) {
                                                                            return searchingCampaign.prizes[j].title;
                                                                        }
                                                                    }
                                                                    return "can not find";
                                                                })()
